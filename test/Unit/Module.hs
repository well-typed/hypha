...
tests = testGroup "Module"
  ...
  , testCase "listExportedSymbols reads fixture file" $ do
      let env = mockBuildEnv "test/fixtures/fake-cabal-store/ghc-9.6.7/async-2.2.5-abc123456/share/async"
  ...
  , testCase "locateSymbolDefinition finds definition line" $ do
      ...
      case loc of
        ...
        Just sl -> do
          slPath sl @?= "test/fixtures/fake-cabal-store/ghc-9.6.7/async-2.2.5-abc123456/share/async/Control/Concurrent/Async.hs"
          slLine sl @?= 1
