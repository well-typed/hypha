...
import Hypha.Error (HyphaError (..), Tool (..), errorCode, errorExitCode)
...
tests = testGroup "Unit.Errors"
  [ testCase "ENOENT from a child-process spawn maps to ToolMissing" $
      case classifyLookupException enoentSpawn of
        ToolMissing _ _ -> pure ()
        other           -> assertFailure ("expected ToolMissing, got: " <> show other)

  , testCase "ToolMissing identifies the haddock binary" $
      case classifyLookupException enoentSpawn of
        ToolMissing ToolHaddock _ -> pure ()
        ToolMissing t _ ->
          assertFailure ("expected ToolHaddock, got: " <> show t)
        other -> assertFailure ("expected ToolMissing, got: " <> show other)

  , testCase "non-IO exception falls back to NetworkError" $
      case classifyLookupException (toException (ErrorCall "boom")) of
        NetworkError _ -> pure ()
        other          -> assertFailure ("expected NetworkError, got: " <> show other)

  , testCase "TOOL_MISSING uses exit code 8" $
      unExitCode (errorExitCode (ToolMissing ToolHaddock placeholderIOE)) @?= 8

  , testCase "TOOL_MISSING is its own wire code" $
      errorCode (ToolMissing ToolHaddock placeholderIOE) @?= "TOOL_MISSING"
  ]
...
enoentSpawn = toException placeholderIOE

-- | Synthetic 'IOException' used by the tests above.  Matches the
-- shape 'readCreateProcessWithExitCode' raises when @posix_spawnp@
-- fails with @ENOENT@ for the requested binary.
placeholderIOE :: IOException
placeholderIOE = IOError
  { ioe_handle      = Nothing
