...
- **One result per definition.** Rows that share a definition site collapse
  ...
  definitions. To search one package, click the "in `<pkg>`" toggle next
  to the search box on any page inside a package, or type `pkg:<name>`
  and a space on any page, which turns the token into that toggle
  (`pkg:aeson decode`). The scope applies before
